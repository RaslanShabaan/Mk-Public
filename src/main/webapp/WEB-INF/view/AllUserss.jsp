...
<div class="wrapper">
    ...
    <h3 style="font-weight: bold;font-style: italic;color: blue;">${del}</h3>    
    <h2>All Site Users </h2>

    <%
    HttpSession sesson=request.getSession();
    ArrayList ff =(ArrayList) sesson.getAttribute("AllUsers");
    int size=ff.size();
%>
 <table style="color: #025aa5;border: 1px solid #000" class="table table-striped tm-full-width-large-table">
    ...
    <tr>
        ...
        <input type="hidden"  name="UserName" value= "<%=  ff.get(count) %>" /> 
        <input type="hidden"name="${_csrf.parameterName}"value="${_csrf.token}"/>
        <button class="btn" style="color:red;font-weight: bold;border-color: red"> Delete </button>
        </form>
        ...
        <input type="hidden"name="${_csrf.parameterName}"value="${_csrf.token}"/>
        <button style="background-color: #888"> Update </button>
        </form>
        -->
        <button class="btn" style="color:green;font-weight: bold;border-color: green"> Update </button>
        </td>
    </tr>
    </tbody>
<%
