...
<%
    ...
    int plus=0;
%>
 <table style="color: #025aa5;border: 1px solid #000" class="table table-striped tm-full-width-large-table" border="1">
    <thead>
    <tr>
    ...
    <tr>
        <td><%=  ff.get(plus+2)    %> </td>
        <td><img style="width:150px;height:100px" src="/images/<%= ff.get(plus+3)  %>" /></td>
        <td><%=  ff.get(plus+1)    %> </td>
        <td>
        <form method="Post" action="/DeleteArticle">
        <input type="hidden"  name="ArtId" value= <%= ff.get(plus) %> /> 
        <input type="hidden"name="${_csrf.parameterName}"value="${_csrf.token}"/>
        <button style="color: red;border-color: red" class="btn"> Delete </button>
        </form>
        </td>
        ...
        <input type="hidden"  name="ArtId" value= <%= ff.get(plus) %> /> 
        <input type="hidden"name="${_csrf.parameterName}"value="${_csrf.token}"/>
        <button style="color:green;border-color: green" class="btn"> Update </button>
        </form>
        </td>
    </tr>
    </tbody>
